# emergency_stop_login_b.ps1
#
# EMERGENCY STOP for Login B (port 9223 / sync Chrome).
#
# Use this when Manheim has locked Login B (the 2026-05-04 incident class)
# and you need to stop all auto-recovery activity for B immediately.
#
# What this does:
#   1. Records a Manheim account-lock for port 9223 (12-hour cooldown by default)
#      so any future login click / browser recovery attempt on B is refused
#      at the disk-backed auth_lockout gate.
#   2. Kills the Chrome process running on port 9223 so its password manager
...
#     stays running. Login A continues normally. The startup gate will
#     refuse to spawn FRESH Python instances while B is locked, but the
#     currently-running Python keeps going.
#   - Does NOT touch Login A's Chrome or state.
#   - Does NOT clear any state. To resume, run scripts/restart_scraper.ps1
#     (which clears the lockout) AFTER you've manually re-authed B via
...
Write-Host ' EMERGENCY STOP — LOGIN B (port 9223)' -ForegroundColor Red
Write-Host '=================================================================' -ForegroundColor Red
Write-Host ''
Write-Host 'This will:'
Write-Host '  1. Set a Manheim account-lock for port 9223 (12-hour cooldown)'
Write-Host '  2. Kill Chrome.exe processes on port 9223'
Write-Host '  3. Show current scraper status'
Write-Host ''
Write-Host 'Login A will continue running. The scraper Python+launcher are NOT stopped.'
Write-Host ''
...
Write-Host ''
Write-Host '[1/3] Recording lockout for port 9223...' -ForegroundColor Yellow
$pyOk = $false
try {
    & python -c "from pathlib import Path; from ove_scraper import auth_lockout; auth_lockout.record_manheim_account_locked(Path('artifacts'), port=9223, reason='operator emergency_stop_login_b.ps1')"
...
Write-Host ''
Write-Host '[2/3] Killing Chrome processes on port 9223...' -ForegroundColor Yellow
...
if ($chromeProcs) {
    ...
    Write-Host ('  Killed {0} Chrome process(es) on port 9223.' -f $count) -ForegroundColor Green
} else {
    Write-Host '  No Chrome processes found on port 9223 (already stopped?).'
}
...
Write-Host '=================================================================' -ForegroundColor Red
Write-Host ''
Write-Host '  1. Confirm with Manheim that Login B is unlocked on their side.'
