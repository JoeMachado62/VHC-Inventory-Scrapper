# manual_login_a.ps1
#
# Launch Login A's Chrome (port 9222 / C:\chrome-cdp-profile profile)
# so you can manually log in to OVE / Manheim and refresh the saved
# credentials.
#
# Use this when:
#   - You've run scripts/emergency_stop_login_a.ps1 (or stop_all)
...
#   2. Wraps the existing scripts/start_ove_browser.ps1 launcher, which
#      handles all the right Chrome flags (debug port, profile dir,
#      session-restore prevention, autosignin disabled).
#   3. Prints what to do in the Chrome window.
#
# Notes:
#   - If a Chrome on port 9222 is already running with the right profile,
#     start_ove_browser.ps1 detects it and does nothing — just bring the
#     existing window to focus and use it.
...
#     time as you. Best practice: stop first via emergency_stop_login_a.ps1.
...
Write-Host ' MANUAL LOGIN — LOGIN A (port 9222)' -ForegroundColor Cyan
Write-Host '=================================================================' -ForegroundColor Cyan
Write-Host ''
Write-Host 'Profile: C:\chrome-cdp-profile'
Write-Host ''
Write-Host 'BEFORE PROCEEDING — confirm:' -ForegroundColor Yellow
Write-Host '  [ ] Manheim has unlocked Login A on their side'
Write-Host '  [ ] You have your Manheim password ready in case Chrome''s'
Write-Host '      saved credential prompt fails'
Write-Host '  [ ] You have run scripts/emergency_stop_login_a.ps1 if the'
...
Write-Host ''
Write-Host 'Launching Chrome via scripts/start_ove_browser.ps1 ...' -ForegroundColor Yellow
...
if ($chrome) {
    Write-Host ('  Chrome on port 9222 is running (PID {0}).' -f ($chrome | Select-Object -First 1).ProcessId) -ForegroundColor Green
} else {
    Write-Host '  WARNING: Chrome on port 9222 not detected after launch.' -ForegroundColor Red
    Write-Host '  Check c:\Users\joema\Auction Module\scripts\start_ove_browser.ps1 manually.'
}
...
Write-Host '       - Confirm the email/password fields auto-fill'
